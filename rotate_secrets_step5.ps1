...
# - AzureCli: production-friendly mode. The Azure CLI context may be backed by
#   managed identity (az login --identity) or workload federation/OIDC.
$AuthenticationMode =
...
if ($AuthenticationMode -notin @("ClientSecret", "AzureCli")) {
    throw "AuthenticationMode '$AuthenticationMode' is invalid. Use ClientSecret or AzureCli."
}
...
if ($AuthenticationMode -eq "AzureCli" -and -not (Get-Command az -ErrorAction SilentlyContinue)) {
    throw "AuthenticationMode AzureCli requires Azure CLI ('az')."
}
...
# AzureCli authentication needs an authenticated CLI context even for Discover
# or -WhatIf, because the Graph token comes from the current Azure identity.
# Real Rotate runs also need Azure CLI for Key Vault operations.
$AzureCliRequired =
